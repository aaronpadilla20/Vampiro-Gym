    ...
    public partial class generalReportWindow : Form
    {
        ...
        private void pictureBoxIpl1_Click(object sender, EventArgs e)
        {
            if(commentsTextBox.Text == "")
            {
                ...
                {
                    generateReport("Sin comentarios");
                }
                else
                {
            ...
            {
                generateReport(commentsTextBox.Text);
            }
           
        }

        private void generateReport(string comentario)
        {
            DialogResult res = MessageBox.Show("Desea generar un reporte general desde el día " + fromDate.Text + " hasta el día " + toDate.Text + " con el siguiente comentario: " + comentario + "?", "Generando reporte", MessageBoxButtons.YesNoCancel, MessageBoxIcon.Question);
